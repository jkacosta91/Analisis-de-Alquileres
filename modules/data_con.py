...
class DataConn:
    def __init__(self, config: dict,schema: str = "jkacosta91_coderhouse"):
        self.config = config
        self.schema = schema
        self.db_engine = None
    ...
    def check_table_exists(self, idealista:str) -> bool:
        with self.db_engine.connect() as connection:
            cursor = connection.cursor
            query_checker = f"""
                SELECT 1 FROM information_schema.tables 
                WHERE  table_schema = 'self.schema'
                AND    table_name   = '{idealista}';              
            """
            cursor.execute(query_checker)

            if not cursor.fetchone():
                logging.error(f"No {idealista} has been created")
                raise ValueError(f"No {idealista} has been created")

            logging.info(f"{idealista} exists")

    def upload_data(self, data: pd.DataFrame, table: str = "idealista"):
